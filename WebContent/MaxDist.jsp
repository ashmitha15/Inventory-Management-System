...
<form name="datepickeer" action="MaxDist.jsp" method="POST">
<table>
...
Class.forName("oracle.jdbc.driver.OracleDriver").newInstance();
           ...
           String queryString = "SELECT D1.DIST_ID, D1.DIST_NAME FROM FALL22_S001_12_DISTRIBUTOR_PROD_DETAILS DP1 , FALL22_S001_12_DISTRIBUTOR D1 WHERE D1.DIST_ID = DP1.DIST_ID AND DATE_OF_PURCHASE BETWEEN ? AND ? GROUP BY D1.DIST_ID, D1.DIST_NAME HAVING COUNT(DISCOUNT)IN(SELECT MAX (COUNT_OF_DISCOUNT_PERCENT) FROM(SELECT D.DIST_ID, DIST_NAME, COUNT(DISCOUNT) AS COUNT_OF_DISCOUNT_PERCENT FROM FALL22_S001_12_DISTRIBUTOR_PROD_DETAILS DP , FALL22_S001_12_DISTRIBUTOR D WHERE D.DIST_ID = DP.DIST_ID AND DATE_OF_PURCHASE BETWEEN ? AND ? GROUP BY D.DIST_ID, D.DIST_NAME ORDER BY COUNT(DISCOUNT) DESC)TEMP)";
           ...
           while (rs.next()) {
               dets.add("Distributor ID: " +rs.getString("DIST_ID"));
               dets.add("Distriutor Name: " +rs.getString("DIST_NAME"));
           }
           request.setAttribute("detsList", dets);
           request.getRequestDispatcher("MaxDist.jsp").forward(request,
                   response);
           %>
           <%
         }
...
%>
<form action="MaxDist.jsp" method="post">
<table border="1" >
